...
목표 항공기의 엔진 배기나 표면 마찰열에서 나오는 MWIR(중파적외선 신호)가 센서로 들어옴.->InSb 반도체 소자는 들어오는 적외선 광자를 전기 신호로 변환.->수많은 픽셀로 이루어진 FPA 덕분에 적외선 이미지를 2D로 만들 수 있음->얻어진 적외선 영상에서 목표를 탐지하고, 플레어 같은 기만 신호와 구별.->트래킹 알고리즘이 프레임 단위로 위치 변화를 계산하여 목표를 계속 추적->시커에서 계산된 목표 방향·각속도 정보가 미사일 제어 시스템으로 전달->미사일은 날개·추력 벡터를 조절하여 목표를 향해 고속으로 유도됨  
